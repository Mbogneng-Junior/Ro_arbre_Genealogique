...
export default function FamilyCreationForm()
{
      ...
      async function handleAddMember()
      {
          ...
          const dataToSend = members.map((member) => {
              const {id, imagePreview,role, ...rest} = member;
              console.log(id, imagePreview,role);
              if (member.role == "parent")
              {
                  return  {
                      poids: 1,
                      targetUsername: localStorage.getItem("username") as string,
                      sourceUsername: member.firstName,
                      familyId: localStorage.getItem("currenCreatedFamilyId") as string,
                      targetUser: {
                          ...rest,
                      }
                  }
              }
              else if (member.role == "enfant")
              {
                  return  {
                      poids: 1,
                      sourceUsername: localStorage.getItem("username") as string,
                      familyId: localStorage.getItem("currenCreatedFamilyId") as string,
                      targetUser: {
                          ...rest,
                      }
          ...
          try
          {
              const response  = await axiosInstance.post("/add_member", dataToSend);
              setIsLoading(false);
              if(response.status === 200)
              {
                  console.log(response?.data);
                  setMembers([]);
                  localStorage.removeItem("currenCreatedFamilyId");
